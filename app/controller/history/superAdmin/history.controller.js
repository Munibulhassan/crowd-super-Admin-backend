...
class History {
  async create(req, res) {
    try {
      // let { name } = req.body;
      // const dataExisted = await historyService.Model.findOne({ name });
      // if (dataExisted)
      //   return res.send({
      //     success: true,
      //     status: 200,
  ...
  async get(req, res) {
    try {
      let { id } = req.params;
      const data = await historyService.Model.findById(id);
      if (!data)
        return res.send({
          success: true,
          status: 200,
  ...
  async gets(req, res) {
    try {
      let data = await historyService.Model.find();
      if (data.length == 0)
        return res.send({
          success: true,
          status: 200,
  ...
  async update(req, res) {
    try {
      let { id } = req.params;
      let data = await historyService.Model.findById(id,{_id:0,name:1,code:1,description:1});
      if(req.body.code){
  ...
  async delete(req, res) {
    try {
      let { id } = req.params;
      let data = await historyService.Model.findById(id,{_id:1});
      if (!data)
        return res.send({
          success: true,
          status: 403,
